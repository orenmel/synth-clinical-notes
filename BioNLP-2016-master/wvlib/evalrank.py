...
def evaluate(wv, reference, use_random):
    ...
    for words, sim in sorted(reference, key=lambda ws: ws[1]):
        try:
            ...
            estimated_sim = cosine(v1, v2)
            good_count += 1
            #print words[0],words[1] ,"\t",v1[0],"\t",v2[0]
        except KeyError:
#             estimated_sim = -1.0
            estimated_sim = use_random.rand()*2-1 # [-1,1)
        
        gold.append((words, sim))
        #print len(variance)

                
        predicted.append((words, estimated_sim)) # this function have problem when v1 v2 are very similar
...
def read_reference(name, encoding=DEFAULT_ENCODING, accepted_words = {}):
    ...
    with codecs.open(name, 'rU', encoding=encoding) as f:
        for line in f:
            ...
            try:
                if len(accepted_words) == 0 or (fields[0] in accepted_words and fields[1] in accepted_words):
